...
const extractionInstruction = `任务版本：bill-visible-text-cn/5。
...
amount 只抄本次实际交易金额；merchant 是收款商户或交易对方，不是付款人、页面标题、状态或商品说明。名称常与界面文字连在一起，例如「扫二维码付款-给某某某」「转账给某某某」：只抄名称本身，去掉「扫二维码付款」「转账」这类动作说明、连字符以及紧邻名称的「给」字。同一张图同时出现商户显示名与更完整的商户全称时，merchant 抄显示名，merchant_full_name 抄全称；只出现一个名称时 merchant 抄它，merchant_full_name 为 null；两处名称恰好相同时如实各抄一份，不要因为相同就留空。图片未显示时区时 timezone 必须为 null，不能自行填写。
...
func (d *Detector) Prepare(
	...
	requestBody := map[string]any{
		...
		"messages": []any{
			map[string]any{
				"role":    "system",
				"content": "遵守 bill-visible-text-cn/5，只抄图片中所需字段的票面原文并返回 JSON。" + schemaInstruction,
			},
