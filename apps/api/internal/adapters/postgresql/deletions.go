...
func (t transaction) DeleteProviderConfig(ctx context.Context, command ports.ProviderDeleteCommand) error {
	...
	result, err := t.tx.ExecContext(ctx, `
		...
		SET active = FALSE, encrypted_api_key = NULL, deleted_at = ?, updated_at = ?, version = version + 1
	...
	if _, err := t.tx.ExecContext(ctx, `
		...
		) VALUES (?, ?, ?, 'provider_config_deleted', 'provider_config', ?, ?, ?::jsonb, ?)
	`,
