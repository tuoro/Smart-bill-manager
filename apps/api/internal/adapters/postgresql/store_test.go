...
func TestBootstrapOwnerRollsBackOnInjectedTransactionFailure(t *testing.T) {
	...
	ctx := context.Background()
	if _, err := store.DB().ExecContext(ctx, `
		CREATE FUNCTION pg_temp.fail_tenant_insert() RETURNS trigger
		LANGUAGE plpgsql AS $$
		BEGIN
			RAISE EXCEPTION 'synthetic_bootstrap_failure';
		END;
		$$;
		CREATE TRIGGER fail_tenant_insert
		BEFORE INSERT ON tenants FOR EACH ROW
		EXECUTE FUNCTION pg_temp.fail_tenant_insert()
	`); err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, 8, 27, 12, 0, 0, 0, time.UTC)
...
func TestMigrationIsIdempotent(t *testing.T) {
	...
	if err := store.db.QueryRow("SELECT current_user").Scan(&runtimeRole); err != nil {
		t.Fatal(err)
	}
	if err := migrate(context.Background(), store.db, migrationsDir(t), runtimeRole); err != nil {
