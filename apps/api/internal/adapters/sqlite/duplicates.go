...
func (t transaction) ListVisualDuplicateDocuments(
	...
	rows, err := t.tx.QueryContext(ctx, `
		WITH candidate_documents AS (
			SELECT DISTINCT target.document_id
			FROM document_pages current
			JOIN document_pages target
			 ...
			 AND target.document_id <> current.document_id
			 AND (
			      target.dhash_band_0 = current.dhash_band_0
			   OR target.dhash_band_1 = current.dhash_band_1
			   OR target.dhash_band_2 = current.dhash_band_2
			   OR target.dhash_band_3 = current.dhash_band_3
			 )
			WHERE current.tenant_id = ? AND current.document_id = ?
		)
...
func (t transaction) listInvoiceDuplicateTargets(
	...
	rows, err := t.tx.QueryContext(ctx, `
		SELECT id, total_minor, currency, invoice_number, invoice_date, seller_name, buyer_name
		FROM invoices
