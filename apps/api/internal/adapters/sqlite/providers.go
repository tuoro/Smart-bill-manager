...
func (t transaction) RecordProviderCapability(
	...
	result, err := t.tx.ExecContext(ctx, `
		...
		SET capability_status = ?,
		    active = CASE WHEN ? = 'passed' THEN active ELSE 0 END,
...
func (t transaction) ActivateProviderConfig(
	...
	if _, err := t.tx.ExecContext(ctx, `
		UPDATE provider_configs SET active = 0, updated_at = ?
	...
	result, err := t.tx.ExecContext(ctx, `
		UPDATE provider_configs SET active = 1, updated_at = ?
