...
func (s *Store) ListReimbursements(
	...
	statement := `
		SELECT reimbursement.id, reimbursement.trip_id,
		       reimbursement.trip_destination, reimbursement.trip_start_date, reimbursement.trip_end_date,
...
func withReimbursementReadSnapshot[T any](
	...
	if err != nil {
		return zero, fmt.Errorf("acquire reimbursement read connection: %w", err)
	}
...
func loadReimbursementPolicyItems(
	...
	rows, err := queryer.QueryContext(ctx, `
		SELECT assignment.id, assignment.payment_id, assignment.invoice_id,
		       payment.merchant, payment.business_date,
		       payment.amount_minor, payment.currency,
		       invoice.seller_name, invoice.invoice_date, invoice.total_minor, invoice.currency
...
func (t transaction) SubmitReimbursement(
	...
	if _, err := t.tx.ExecContext(ctx, `
		...
		) VALUES (?, ?, ?, 'reimbursement_submitted', 'reimbursement', ?, ?, ?, ?)
...
func loadReimbursementItems(
	...
	rows, err := queryer.QueryContext(ctx, `
		SELECT item.id, item.trip_fact_assignment_id, item.fact_type,
		       ...
		       END,
		       item.display_name, item.business_date, item.amount_minor,
...
func reimbursementWriteError(operation string, err error) error {
	message := err.Error()
	switch {
	case strings.Contains(message, "reimbursements.tenant_id, reimbursements.trip_id"):
	...
	case strings.Contains(message, "reimbursement_status_decisions.tenant_id, reimbursement_status_decisions.idempotency_key"):
