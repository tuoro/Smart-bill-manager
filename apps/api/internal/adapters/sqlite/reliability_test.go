...
func TestPersistentQueueLeasesFiftyJobsExactlyOnce(t *testing.T) {
	...
	for index := 0; index < 50; index++ {
		job, err := store.LeaseNextJob(ctx, "worker", now, now.Add(165*time.Second))
		if err != nil {
		...
		seen[job.ID] = struct{}{}
		if job.AttemptCount != 1 || job.LeaseOwner != "worker" {
...
func TestProviderActivationRequiresSchemaIdentity(t *testing.T) {
	...
	if active != 0 || status != "failed" {
		t.Fatalf("failed re-detection state = active:%d status:%s", active, status)
	}
}
