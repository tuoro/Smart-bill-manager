	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			validated := ValidateClaim(ClaimEnvelope{
				SchemaVersion: "document-claim/4", DocumentType: "invoice", Fields: base,
			}, test.pages)
...
func TestValidateInvoicePageTopologyAllowsIndependentPagesAndThreePageSpan(t *testing.T) {
	...
	validated := ValidateClaim(ClaimEnvelope{SchemaVersion: "document-claim/4", DocumentType: "invoice", Fields: fields}, 3)
