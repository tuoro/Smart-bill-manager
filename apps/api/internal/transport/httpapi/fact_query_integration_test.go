...
func TestHTTPFactManagementQueryDetailsAndSourceBoundary(t *testing.T) {
	...
	for _, session := range []*testSession{owner, finance} {
		...
		if !hasSource || source.(map[string]any)["claim_set_id"] != review["claim_set_id"] {
	...
	assertStatus(t, f.request(http.MethodGet, "/api/v1/documents/"+documentID+"/content", nil, finance, false, ""), http.StatusOK)
	...
	assertStatus(t, f.request(http.MethodGet, path+"?include_source=true", nil, finance, false, ""), http.StatusBadRequest)
