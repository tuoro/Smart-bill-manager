...
test('重复与关联必须逐单决定；blocked 暂缓不计入保存', async ({ page }) => {
  ...
  await page.getByLabel('本次分配（最小单位）').fill('100')
...
test('异常定位和草稿保护，浅深主题与四档宽度', async ({ page }, testInfo) => {
  ...
  await page.getByLabel('支付金额（最小单位）', { exact: true }).fill('12345')
  await page.getByRole('button', { name: '支付金额（最小单位） 清空证据选择', exact: true }).click()
  ...
  await expect(page.getByLabel('支付金额（最小单位）', { exact: true })).toHaveValue('12345')
