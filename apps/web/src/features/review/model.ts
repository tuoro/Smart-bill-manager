...
import { isRealDate, parseRFC3339 } from '../facts/time'
...
// 来源时区取用户当前编辑中的值：同一次修订里时区和时间可能一起改，
// 用旧时区显示会让人按一个已经不成立的读数去核对。
export function sourceTimezone(fields: EditableField[]): string {
  const field = fields.find((entry) => entry.path === 'source_timezone')
  return field?.presence === 'present' && field.textValue ? field.textValue : 'UTC'
}
...
export function buildFieldPayload(
  ...
  for (const field of fields) {
    ...
    if (field.valueType === 'money_minor') {
      ...
      value = parsed.minor
    } else if (field.valueType === 'instant') {
      // 与后端 time.Parse(time.RFC3339Nano) 同一口径：手输一个 2026-9-4 这样的
      // 写法，应该在输入处就说清楚，而不是提交后收到一句通用拒绝。
      if (!parseRFC3339(field.textValue)) {
        errors[field.path] = '时间必须是 RFC3339，例如 2026-09-04T16:00:00+08:00'
        continue
      }
      value = field.textValue
    } else if (field.valueType === 'date') {
      if (!isRealDate(field.textValue)) {
        errors[field.path] = '日期必须是 YYYY-MM-DD，例如 2026-09-04'
        continue
      }
      value = field.textValue
...
// 只读展示与编辑同源：来源时区取 Claim 里的值，缺失时按 UTC 而不是浏览器时区
// ——账单的本地时间是票面所在地的时间，跟看账单的人在哪没有关系。
export function reviewSourceTimezone(review: Review): string {
  const field = review.fields.find((entry) => entry.path === 'source_timezone')
  return field?.presence === 'present' && typeof field.value === 'string' ? field.value : 'UTC'
}
