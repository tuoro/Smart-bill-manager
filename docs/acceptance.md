...
[ADR-0036](decisions/0036-review-allocation-throughflow.md#验收结果) 联合验收已本地通过：三张合成单据经过真实上传、处理、连续审核成为 Fact；包含 A 的一次明确重复候选过期冲突、零部分写入及已有 UI 的显式未改字段修订恢复，不称首次直接成功。歧义不自动分配，先手工 4000、再草案补充 6000，重读双端余额与完整 Link，确认旧关系未改写；真实数据库核对 3 个 AiRun、4 个 Claim、3 条 Review、2 条有效 Link、2 次补充分配及完整来源链。115 项 Web 单元、10 项 Provider 单元、1 项真实浏览器贯通、静态检查和生产构建通过，前三次失败保留，临时资源已清理；见 [安全聚合](../tests/evidence/review-allocation/throughflow.json)。固定冲突外其他 API 异常均为 0，初始未登录 401 单独记录；不把 Mock API 历史结果、脚本耗时或固定模拟提取当成真实模型质量和用户时间收益。

分配草案已按 [ADR-0035](decisions/0035-allocation-suggestion-draft.md#验收结果) 完成本地验收：固定 8 个合成场景的实际指定表单操作 40→27，完整计划一致，理由和最终保存各保持 8 次；115 项单元、150 项 Mock 浏览器及静态检查/生产构建通过。歧义不默认接受，已有 Link、跨期手工、409 与未知结果同键重试均保留；[安全聚合](../tests/evidence/allocation-suggestion/allocation-draft.json) 不作为真实用户耗时或模型正确率证据。

当前审核减负首切片已按 [ADR-0034](decisions/0034-continuous-review-reduction.md#验收协议) 完成本地验收：相同 20 单合成点击 79→21，正常单据一次明确保存、连续队列及失败不推进；56 项单元与 136 项 Mock 浏览器测试通过，证据见 [安全聚合](../tests/evidence/review-reduction/continuous-review.json)。不把合成操作次数或脚本耗时当作真实模型正确率/用户节省时间；真实 30% 时间减负目标仍待端到端测量。原有字段/重复/金额/权限/幂等与人工 Review 门禁不降低。
...
- M1 支付默认币种固定为 `CNY`，边界见 [ADR-0037](decisions/0037-default-payment-currency.md)：显式币种字段与金额文本推导都拿不到币种时套用，且不写入 Evidence——与默认时区同为产品默认值，不得伪装成票面逐字证据。金额文本出现 `$`、`€`、`USD`、`美元` 等标记时按票面解析，默认值不得覆盖；显式币种与金额文本冲突仍按 `money_currency_conflict` 阻断。发票区段不套用默认币种。
...
- 关键字段证据覆盖率的分母是本地接受结果中的全部关键字段；页码必须一致，且 quote 或 region 至少一个能直接支持冻结期望值。冻结的较长 quote 不是必须逐字复现：值本身的最小充分片段可以通过，例如 `¥` 支持 CNY，`¥68.00` 支持长金额片段；无关或仅部分相同的数字不能通过。文档级失败不伪造字段证据。来源标记为产品默认值的关键字段不进入该分母（当前为 `m1-payment-timezone/1` 与 `m1-payment-currency/1`，后者见 [ADR-0037](decisions/0037-default-payment-currency.md)）：产品默认值不是模型读数，要求它提供票面证据只会逼出伪造证据；豁免只认清单 `derived_field_provenance` 里的这两个批准标识，其余取值一律照常计入。
...
- 当前 8 个 Playwright spec 的完整场景均必须执行且至少保持 73 项通过（`v0.3.4` 增补导航与全站 UI 回归）；不得用删除场景、只跑组件矩阵或复用历史 M1 结果替代当前镜像 E2E。历史 M4 六文件结果保留为当时证据，UI 补丁本次验收范围见 [v0.3.4](releases/v0.3.4.md)。
