...
**钉钉收单（可选）。** 不需要任何环境变量。管理员在「系统 → 钉钉收单」页填入钉钉开放平台应用的 AppKey / AppSecret，点「检测」验证凭据，再「启用」。AppSecret 用主密钥加密落库，界面只回显「已设置」。启用后进程向钉钉发起一条出站长连接（Stream 模式），**不需要公网入站**；停用或更换凭据即时生效，不必重启。成员在「账号与密码」页生成绑定码发给机器人完成绑定，之后把图片或 PDF 直接发给机器人即进入识别队列。凭据按工作区保存，同一个 AppKey 不能被两个工作区使用。建应用、绑定账号和聊天里可用的回复见[钉钉收单配置](dingtalk.md)。
