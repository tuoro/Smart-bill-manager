...
安装器先检查 Docker 是否可用、Docker 服务端架构是否为 `amd64`、Compose 版本和可用内存——不满足条件时在提问之前就失败，不会先让你回答一轮再报错。当前镜像只发布 `linux/amd64`，在 ARM 主机上会在这一步明确拒绝。

随后只显示一屏确认信息——数据保存在哪、装完访问哪个地址——按一次 Enter 即可开始。想换位置就直接输入完整路径。加 `--yes` 可完全跳过交互；没有可交互终端时（CI、`ssh host 'curl ... | sh'`）安装器会说明情况并按默认值继续，而不是中止。

端口不再询问：默认 `8080`，被占用时自动向后选一个空闲端口并在确认信息里说明。

数据库数据、上传的单据和备份默认都放在数据保存位置下面，普通安装无需分别指定。需要分盘或 NAS 时用 `--postgres-directory`、`--objects-directory`、`--backups-directory` 显式给出：路径必须是绝对路径，父目录必须已存在，三个目标不能相同；安装器不会覆盖或接管已有目录。

数据保存位置不能放在部署包目录内。流式安装的部署包解压在临时目录，安装结束会被清理，放在里面会连数据一起丢失；安装器对此显式拒绝。

配置完成后，安装器**先拉取镜像，再创建任何目录**。这一步依赖外部网络且需要下载数百 MiB，是整个流程里最容易被中断的一环；把它放在前面意味着中断时磁盘上不留痕迹，直接重新运行即可。随后按固定顺序完成 owner-only secret 创建、PostgreSQL provision、Schema migration、应用启动和状态检查。

安装完成后，部署包会被复制到 `<数据保存位置>/bundle`，并在数据保存位置下生成一个 `sbm` 包装脚本，日常管理不必重复冗长路径：

```bash
~/smart-bill-manager/sbm status
~/smart-bill-manager/sbm stop
~/smart-bill-manager/sbm start
```

流式安装的临时目录随后被清理，这些命令依然可用。

如果在拉取之后的某一步失败（例如 provision 中断），运行目录会被保留。此时用同一个运行目录重新运行安装器即可续跑：它检测到 `deployment.env` 后沿用原有配置和 secret，不再重复提问，也不会重新生成主密钥。若目标目录已存在但不含 `deployment.env`，安装器拒绝接管并提示更换目录。PostgreSQL 与应用始终是两个独立容器，数据库不发布宿主端口。
