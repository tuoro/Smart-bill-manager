...
安装器先检查 Docker、Compose 版本和可用内存——不满足条件时在提问之前就失败，不会先让你回答一轮再报错。
...
配置完成后，安装器**先拉取镜像，再创建任何目录**。镜像拉取需要访问 `ghcr.io`，是最常见的失败点；把它放在前面意味着失败时磁盘上不留痕迹，直接重新运行即可。随后按固定顺序完成 owner-only secret 创建、PostgreSQL provision、Schema migration、应用启动和状态检查。
