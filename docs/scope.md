...
M1 已于 2026-08-30 完成。清晰、完整、无遮挡且关键字段可直接人工辨读的原始输入是当前质量承诺；模型正确率高目标保留为全部功能开发完成后的 M4 正式发布门禁。安全闭环、人工审核、Source/Claim/Fact、租户隔离和确定性规则没有例外；收口证据见 `docs/m1-evidence.md`。M2 已获明确授权并处于进行中，支付—发票金额分配、确定性重复检测、跨页明细审核和多 Document 批量上传四个切片均已完成，证据见 `docs/m2-evidence.md`。
...
进入条件已满足：M1 全部门禁通过，产品负责人于 2026-08-30 明确授权 M2。前四个切片已经按冻结范围实现并通过自动化验收；当前断点是先冻结“已确认 Fact 之间的独立补充分配、撤销或替换工作流”的范围、失败边界、状态机和必要 ADR。M2 整体仍未完成；不得在文档冻结前实施下一切片，也不得远端推送、部署或增加旧数据兼容。
