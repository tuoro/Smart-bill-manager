...
M1 已于 2026-08-30 完成。清晰、完整、无遮挡且关键字段可直接人工辨读的原始输入是当前质量承诺；模型正确率高目标保留为全部功能开发完成后的 M4 正式发布门禁。安全闭环、人工审核、Source/Claim/Fact、租户隔离和确定性规则没有例外；收口证据见 `docs/m1-evidence.md`。M2 已获明确授权并处于进行中，首个支付—发票金额分配切片已完成，证据见 `docs/m2-evidence.md`。
...
- M3、M4 功能、部署、提交、推送或旧数据兼容。
- 跨币种分配、汇率换算、自动截断余额或自动接受候选；
- 在本首切片中原地修改活动 Link 金额，或为同一 Payment/Invoice 对创建多条并行活动 Link。

### 进入条件

进入条件已满足：M1 全部门禁通过，产品负责人于 2026-08-30 明确授权 M2，并要求先冻结首切片范围与验收再实施。首切片已经按冻结范围实现并通过自动化验收；M2 整体仍未完成。下一开发断点是先冻结后续切片的范围、验收和必要架构决策，当前顺序从近似文件、字段组合和跨页重复检测开始；不得提前实现 M3、M4、部署或旧数据兼容。
