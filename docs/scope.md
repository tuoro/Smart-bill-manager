...
当前已完成本地验收的新增范围为 [ADR-0035](decisions/0035-allocation-suggestion-draft.md)：已确认支付/发票的调整分配页生成非持久草案，显式采用后可改，理由与最终确认不省略。仅完整初始 30 天候选可准备建议；多目标竞争不代选，部分金额留空，不改既有关联或初次 Review 规则，不增模型/API/Schema/发布范围。下面 ADR-0034 是已完成的前一切片，两者均未提交或发布。
...
B7/B8 已本地验收：按 ADR-0031/0032 实现跨 30 天的有理由手工分配，以及 Payment/Invoice 坏账标记和关联行程删除保护。此前关于这两项待决策的限制被本次明确授权替代；邮箱连接器当时仍在范围外，已于 `v0.6.0` 接通。该切片验收当时未提交、发布或升级用户实例；后续分发以 v0.4.0 的明确授权为准。
