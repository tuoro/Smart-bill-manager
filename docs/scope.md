...
已发布范围以 [v0.6.0 发布记录](releases/v0.6.0.md) 为准：角色收敛为管理员与成员两档，邮箱来源按成员归属并接通真实 IMAP，钉钉收单可在聊天里完成改字段、处理重复件、分配与确认/作废，已确认关联的支付与发票互相跟随行程归属。

仍不在范围内：真实模型正确率的正式评测、大规模真实信箱联调、TLS/域名与生产部署、公开注册与 SSO、为报销连接外部系统。

下面按时间累积的切片记录保留原貌，用于追溯每次授权的边界，不改写成今天的结论。其中出现的 `finance` / `reviewer` / `viewer` 是迁移 `0014` 之前的角色名，现已并入 `member`；`admin` 即今天的 `owner`。

[ADR-0036](decisions/0036-review-allocation-throughflow.md#验收结果) 已完成隔离的当前 Web＋既有真实后端组合贯通：固定三张合成单据、本地模拟 Provider、独立空数据库，经上传、含一次显式冲突恢复的连续审核、分配草案、保存和重读验收。未改产品业务逻辑，临时资源已清理；不是行程新功能、真实模型评测、用户实例升级或发布授权。

已发布切片（`v0.5.0`）：[ADR-0035](decisions/0035-allocation-suggestion-draft.md)：已确认支付/发票的调整分配页生成非持久草案，显式采用后可改，理由与最终确认不省略。仅完整初始 30 天候选可准备建议；多目标竞争不代选，部分金额留空，不改既有关联或初次 Review 规则，不增模型/API/Schema/发布范围。下面 ADR-0034 是前一切片，两者均随 `v0.5.0` 发布。

当前新增范围仅为 [ADR-0034](decisions/0034-continuous-review-reduction.md) 审核减负首切片：Web 队列内连续审核、无候选时明确的一次保存确认、通过项折叠和异常定位。没有新增自动关联/入账、后端接口或持久化状态；其队列是有界内存工作集，不替代服务端 Job。当前候选为空时，最终“确认保存，不分配”替代额外单选；有候选/重复/冲突时原有人工决定边界不变。

B7/B8 已本地验收：按 ADR-0031/0032 实现跨 30 天的有理由手工分配，以及 Payment/Invoice 坏账标记和关联行程删除保护。此前关于这两项待决策的限制被本次明确授权替代；邮箱连接器当时仍在范围外，已于 `v0.6.0` 接通。该切片随 `v0.4.0` 发布。
...
B5 当前增补：[ADR-0029](decisions/0029-member-account-lifecycle.md) 授权实现本租户邀请/加入、四角色与停用/恢复、自助改密、多工作区登录和受控本地账号恢复；不增加公开注册、邮件、SSO 或 Owner 跨租户重置密码。`0007` 保留当前数据；随 `v0.4.0` 发布。其中的「四角色」已由迁移 `0014` 并入两档。

状态（写于 B8 切片）：M0～M4 历史冻结范围及 B1～B8 已按 ADR-0025～0032 完成本地验收；真实模型评测、真实外部联调、单列待决策能力和生产部署仍不在本结论内
适用范围：Clean Slate 新架构

历史 `v0.3.4` UI 补丁发布范围与验收见 [发布记录](releases/v0.3.4.md)；该段写于 `v0.4.0`，当前发布范围以 [v0.6.0](releases/v0.6.0.md) 为准，不把历史测量改写为新镜像结果。

已发布切片（`v0.4.0`）：[ADR-0024](decisions/0024-manual-trip-workspace.md) 人工行程创建/编辑、多凭证、唯一时间自动归属与人工优先已通过本地验收；保留 v0.3.4 数据、人工决定和报销快照。随 `v0.4.0` 发布。

已发布切片（`v0.4.0`）：[业务能力补齐计划](functional-completion-plan.md) 的 B1～B6 已依次完成验收，报销材料 ZIP 按 ADR-0030 实现，不能理解为恢复旧导出模块。B7/B8 已单独授权并按 ADR-0031/0032 完成本地验收；邮箱采集仍单列待决策。
...
### 第五切片范围内：v0.3.1 自托管公开实测分发

- 复用已通过 M4 门禁且发布输入摘要不变的 `linux/amd64` 候选，发布带不可变 digest 的版本化 GHCR 镜像，不发布 `latest`；已发布的 `v0.3.0` Git Tag 不重写，部署修复使用 `v0.3.1`；
- 在现有唯一 Compose 契约上增加发布 overlay、owner-only 凭据准备器和最小部署命令，不复制业务服务或安全配置；
- 默认只绑定 `127.0.0.1`，固定应用与 PostgreSQL 镜像 digest，保持 PostgreSQL internal 网络、只读根、最小 capability、资源上限和文件型 secret；
- 完成全新空目录、空数据库、空对象卷的 provision、migration、Owner 初始化、ready、认证和会话失效冒烟；
- 重写中英文 README，并提供面向使用者的部署、启动、停止、升级边界、备份入口和故障诊断说明；
- 发布 `v0.3.1` GitHub 预发布并把 `v0.3.0` 标记为已被替代，明确 amd64、Clean Slate 和未完成门禁。

### 第五切片范围外

- 真实模型正确率评测、真实图片发送、真实邮箱或 Provider 联调；
- TLS、域名、反向代理、远程 PostgreSQL、HA、对象存储、云资源或生产服务器部署；
- arm64 或其他架构镜像、自动更新、`latest` 标签、旧数据迁移或旧版兼容；
- 删除现有业务数据、Docker 卷或用户凭据目录。

本切片以 `docs/decisions/0021-v0.3.1-self-hosted-prerelease-distribution.md` 为冻结边界。

验收结果：2026-09-01 通过。GHCR `v0.3.0` amd64 镜像已公开并可匿名按 digest 拉取；`v0.3.1` 源码补丁保持应用发布输入不变，补齐发布 overlay、仓库外凭据准备、部署命令、部署指南和精简 README。隔离空库部署与认证链通过，临时容器、网络、卷、凭据和原始运行材料均已销毁。

### 第六切片范围内：引导式安装与自定义持久化目录

- 在不改变唯一 Compose 契约的前提下增加一次性引导安装入口，顺序完成准备、拉取、Owner bootstrap、启动和状态检查；
- 支持从显式固定 Git Tag 通过单行命令取得安装器、下载并校验同版本 Release Bundle；
- 允许用户分别指定全新的 PostgreSQL 数据、对象文件和备份绝对路径，缺省时继续使用运行目录下的标准布局；
- 保持应用与 PostgreSQL 独立容器、数据库 internal 网络、固定镜像摘要、文件型 secret、最小权限角色和版本化 migration；
- 更新最小 Release allowlist、README、部署文档和自动化脚本测试。
- README 同时展示一键安装、显式 Docker Compose 和仅用于已初始化部署的 `docker run` 应用容器形式。

### 第六切片范围外

- 把 PostgreSQL 内置到应用镜像、增加第二 Compose、运行时数据库选择或外部 PostgreSQL；
- 把 `docker run` 应用示例伪装成能创建数据库、角色、Schema 或 Owner 的完整单容器安装；
- 接管、覆盖或自动迁移已有目录、数据库、对象文件或旧架构数据；
- 自动更新、`latest`、arm64、TLS、域名、反向代理、生产部署、推送、Tag、Release 或镜像发布；
- 真实模型评测、真实 Provider/邮箱联调或任何业务功能变更。

本切片以 `docs/decisions/0023-guided-self-hosted-installer.md` 为冻结边界。

验收结果：2026-09-01 通过。引导式安装器、固定版本流式下载、Bundle 成功/错误摘要、临时目录回收、默认/自定义目录、端口、唯一 Compose 调用顺序、13 文件部署包及 README 三种部署形式均通过；没有修改业务代码、数据库 Schema、镜像内容或已发布 `v0.3.2` 资产。
