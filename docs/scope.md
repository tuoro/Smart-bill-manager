...
状态：已获持续本地开发授权，尚未开始实施。

当前范围顺序固定为邮箱 Source、邮件与附件、行程归属、报销状态和确定性冲突提示。下一切片只先冻结“邮箱 Source、邮件与附件本地归档”的范围、数据模型、失败边界、API/UI 与验收；本地实现必须使用纯合成、可重复输入。真实邮箱账号、OAuth/密码、云服务连接和实联调均在当前授权之外。
