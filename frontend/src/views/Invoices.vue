...
const {
  ...
  fetchPage: async ({ limit, offset, signal }) => {
    const params: Parameters<typeof invoiceApi.getAll>[0] = { limit, offset }
    ...
    const response = await invoiceApi.getAll(params, { signal })
    if (!response.data.success || !response.data.data) {
      throw new Error(response.data.message || '\u52A0\u8F7D\u53D1\u7968\u5217\u8868\u5931\u8D25')
    }
    return response.data.data
  },
  onError: (error) => {
    toast.add({ severity: 'error', summary: getApiErrorMessage(error, '\u52A0\u8F7D\u53D1\u7968\u5217\u8868\u5931\u8D25'), life: 3000 })
  },
})
