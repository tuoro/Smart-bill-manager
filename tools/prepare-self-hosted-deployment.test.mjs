...
test("deployment preparation creates distinct owner-only secrets without printing them", async () => {
  ...
  try {
    ...
    assert.match(
      environment,
      /^SBM_IMAGE=ghcr\.io\/tuoro\/smart-bill-manager:v0\.3\.0@sha256:83bd3c795b3a7c2413a8f80279ab3fc8b9787e0e9b02cab5b08711c61d3ba6d1$/m,
    );
    assert.match(environment, /^SBM_BIND_ADDRESS=127\.0\.0\.1$/m);
    assert.match(
      environment,
      /^SBM_RELEASE_ARTIFACTS_SOURCE=\/nonexistent\/disabled-by-release-overlay$/m,
    );
  } finally {
